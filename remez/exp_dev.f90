...
! Array-based exp with SVML-style fixup pattern
pure subroutine exp_dev_array(x, a)
  real(kind=real64), intent(in) :: x(:)
  real(kind=real64), intent(out) :: a(:)
  ...
  real(kind=real64) :: r, x_ln2, K, e, Z
  integer(int64) :: eb, kb, xb, iz, kz, j, fb
  ...
  ! First pass: compute fast path for all elements
  do i = 1, n
    x_ln2 = x(i) * TABLE_INV_LN2
    ...
    r = (x(i) - Z * TABLE_LN2_HI) - Z * TABLE_LN2_LO
  ...
  ! Check if any element needs fixup (Inf or extreme K)
  any_extreme = .false.
  do i = 1, n
    xb = transfer(x(i), int64_mold)
    if (iand(xb, abs_mask) >= pos_inf_bits) then
      any_extreme = .true.
      exit
    end if
    x_ln2 = x(i) * TABLE_INV_LN2
    ...
    kz = iand(iz, not(index_mask))
    K = (transfer(kz, real64_mold) - round_bias) * I_NTABLE
    if (abs(K) > 1020.0_real64) then
      any_extreme = .true.
      exit
    end if
  end do

  ! Second pass: fixup only if needed
  if (any_extreme) then
    do i = 1, n
      xb = transfer(x(i), int64_mold)
      ...
      ! Handle Inf
      if (iand(xb, abs_mask) == pos_inf_bits) then
        a(i) = merge(0._real64, transfer(pos_inf_bits, real64_mold), xb < 0)
        cycle
      end if

      ! Recompute K for this element
      x_ln2 = x(i) * TABLE_INV_LN2
      Z = (x_ln2 + round_bias) - round_bias
      iz = transfer(Z + round_bias, int64_mold)
      kz = iand(iz, not(index_mask))
...
end subroutine exp_dev_array


elemental function exp_dev_cr(x) result(a)
  !$acc routine seq
...
elemental function exp_dev_fast(x) result(a)
  !$acc routine seq
  real(kind=real64), value, intent(in) :: x
    !< Input value [nondim]
  real(kind=real64) :: a
